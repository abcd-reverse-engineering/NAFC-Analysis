package h.x2;

import androidx.exifinterface.media.ExifInterface;
import h.q2.t.i0;
import java.util.Iterator;

/* compiled from: Sequences.kt */
@h.y(bv = {1, 0, 3}, d1 = {"\u0000\"\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\u0010\b\n\u0002\b\u0002\n\u0002\u0010(\n\u0000\b\u0000\u0018\u0000*\u0004\b\u0000\u0010\u0001*\u0004\b\u0001\u0010\u00022\b\u0012\u0004\u0012\u0002H\u00020\u0003B-\u0012\f\u0010\u0004\u001a\b\u0012\u0004\u0012\u00028\u00000\u0003\u0012\u0018\u0010\u0005\u001a\u0014\u0012\u0004\u0012\u00020\u0007\u0012\u0004\u0012\u00028\u0000\u0012\u0004\u0012\u00028\u00010\u0006¢\u0006\u0002\u0010\bJ\u000f\u0010\t\u001a\b\u0012\u0004\u0012\u00028\u00010\nH\u0096\u0002R\u0014\u0010\u0004\u001a\b\u0012\u0004\u0012\u00028\u00000\u0003X\u0082\u0004¢\u0006\u0002\n\u0000R \u0010\u0005\u001a\u0014\u0012\u0004\u0012\u00020\u0007\u0012\u0004\u0012\u00028\u0000\u0012\u0004\u0012\u00028\u00010\u0006X\u0082\u0004¢\u0006\u0002\n\u0000¨\u0006\u000b"}, d2 = {"Lkotlin/sequences/TransformingIndexedSequence;", ExifInterface.GPS_DIRECTION_TRUE, "R", "Lkotlin/sequences/Sequence;", "sequence", "transformer", "Lkotlin/Function2;", "", "(Lkotlin/sequences/Sequence;Lkotlin/jvm/functions/Function2;)V", "iterator", "", "kotlin-stdlib"}, k = 1, mv = {1, 1, 16})
/* loaded from: classes2.dex */
public final class y<T, R> implements m<R> {

    /* renamed from: a, reason: collision with root package name */
    private final m<T> f16662a;

    /* renamed from: b, reason: collision with root package name */
    private final h.q2.s.p<Integer, T, R> f16663b;

    /* compiled from: Sequences.kt */
    public static final class a implements Iterator<R>, h.q2.t.q1.a {

        /* renamed from: a, reason: collision with root package name */
        @i.c.a.d
        private final Iterator<T> f16664a;

        /* renamed from: b, reason: collision with root package name */
        private int f16665b;

        a() {
            this.f16664a = y.this.f16662a.iterator();
        }

        public final void a(int i2) {
            this.f16665b = i2;
        }

        public final int b() {
            return this.f16665b;
        }

        @i.c.a.d
        public final Iterator<T> c() {
            return this.f16664a;
        }

        @Override // java.util.Iterator
        public boolean hasNext() {
            return this.f16664a.hasNext();
        }

        @Override // java.util.Iterator
        public R next() {
            h.q2.s.p pVar = y.this.f16663b;
            int i2 = this.f16665b;
            this.f16665b = i2 + 1;
            if (i2 < 0) {
                h.g2.y.f();
            }
            return (R) pVar.invoke(Integer.valueOf(i2), this.f16664a.next());
        }

        @Override // java.util.Iterator
        public void remove() {
            throw new UnsupportedOperationException("Operation is not supported for read-only collection");
        }
    }

    /* JADX WARN: Multi-variable type inference failed */
    public y(@i.c.a.d m<? extends T> mVar, @i.c.a.d h.q2.s.p<? super Integer, ? super T, ? extends R> pVar) {
        i0.f(mVar, "sequence");
        i0.f(pVar, "transformer");
        this.f16662a = mVar;
        this.f16663b = pVar;
    }

    @Override // h.x2.m
    @i.c.a.d
    public Iterator<R> iterator() {
        return new a();
    }
}
